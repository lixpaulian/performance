...
#if MAIN_HAS_NOARGC
MAIN_RETURN_TYPE core_main(ee_u32 iterations) {
	int argc=0;
	char *argv[1];
#else
...
#if HAS_FLOAT
		if (known_id==3) {
			ee_printf("CoreMark 1.0 : %f / %s %s",default_num_contexts*results[0].iterations/time_in_secs(total_time),COMPILER_VERSION, "no-compiler-flags"/* COMPILER_FLAGS*/);
